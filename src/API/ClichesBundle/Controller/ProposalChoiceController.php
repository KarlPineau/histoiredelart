...
class ProposalChoiceController extends Controller
{
    public function indexAction($playerOeuvre_id, Request $request)
    {
        ...
        $difficultyLevel = $this->get('cliches_player.playerproposalchoiceaction')->getDifficultyLevel($playerProposal);
        ...
        foreach($listFields as $key => $fieldContainer) {
            if($this->get('data_data.entity')->valueByField($fieldContainer['field'], $entity) != 'empty' AND
                ...
                $choicesArray = array();
                $choices = $this->get('cliches_player.playerproposalchoiceaction')->getValuesAction($playerOeuvre, $fieldContainer['field'], $difficultyLevel);
    ...
    public function generateMessengerFormat($playerProposal, $teaching, $entity, $view, $propertyContainer, $values, $difficultyLevel, Request $request)
    {
        ...
        $messages = [
            "messages" => [
                //["text" => 'Niveau de difficulté: '.$difficultyLevel],
                [
                    "attachment" => [
                        "type" => "image",
                        "payload" =>  [
                ...
                [
                    "text" => $propertyContainer['label'].' de cette oeuvre ?',
